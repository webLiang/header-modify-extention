...
id: github-release
category: Release
description: Build ZIP, commit, tag, push, and create GitHub Release (webLiang/header-modify-extention)
---

Publish a GitHub Release: https://github.com/webLiang/header-modify-extention/releases

**Script**: `scripts/github-release.mjs`  
**One-shot command**: `pnpm release:github:full` (= build → zip → notes → commit → tag → gh release → push)  
...
1. `pnpm build:zip` (`pnpm build` + `pnpm zip`)
...
5. `gh release create` with `.zip` (and `.crx` if present), release notes
...
| `--title "v1.0.0"` | Override GitHub release title |
| `--asset <path>` | Extra attachment (repeatable) |
| `--skip-build` | Skip `build:zip` |
...
- **Do not** use `--full` if `package.json` version was not bumped (except the first release when no `v*` tags exist).
